...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchErrorBarcodeCorrection' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchErrorBarcodeCorrection
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchErrorBarcodeCorrection] 


AS
BEGIN
	SELECT  DISTINCT (SP.[FirstName] +' '+ SP.[LastName]) AS SubjectName
		,SP.[UniqueSubjectID] AS SubjectID
		,SPD.[RCHID]
		,CONVERT(VARCHAR,SPD.[LMP_Date],103) AS LMPDate
		,EB.[AlterNateBarcode] AS BarcodeNo
	...
	WHERE  EB.[ProblemSolvedStatus] = 0
