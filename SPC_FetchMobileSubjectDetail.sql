...
BEGIN
	...
	AND (SP.[SubjectTypeID] = 1 OR SP.[SubjectTypeID] = 2 OR SP.[SubjectTypeID] = 3) 
	AND  SPAD.[UniqueSubjectID] IN (SELECT UniqueSubjectID  FROM Tbl_SubjectPrimaryDetail WHERE 
	SP.[SubjectTypeID] = 1 OR SP.[SubjectTypeID] = 2 OR SP.[SubjectTypeID] = 3)
	UNION
	SELECT 
		SP.[SubjectTypeID]
		,SP.[ChildSubjectTypeID]
	...
	LEFT JOIN [dbo].[Tbl_SubjectPregnancyDetail] SPD WITH (NOLOCK) ON SP.[ID] = SPD.[SubjectID]
	LEFT JOIN [dbo].[Tbl_SubjectParentDetail] SPAD WITH (NOLOCK) ON SP.[ID] = SPAD.[SubjectID]
	WHERE SP.[AssignANM_ID] = @UserId  AND SP.[SubjectTypeID] = 4 -- AND SP.[UpdatedToANM] = 1
	AND  SPAD.[UniqueSubjectID] IN (SELECT UniqueSubjectID  FROM Tbl_SubjectPrimaryDetail WHERE 
	SP.[SubjectTypeID] = 4) AND SP.[UpdatedToANM] = 1
