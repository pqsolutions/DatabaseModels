...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='FN_GenerateANMShipmentId' AND [type] = 'FN')
BEGIN
...
BEGIN
	...
	SET @LastShipmentId = (SELECT TOP 1 ShipmentID 
							FROM Tbl_ANMShipment WITH(NOLOCK) WHERE RIID = @SenderId  AND ShipmentID LIKE '%'+@Source   
