...
BEGIN
	DROP FUNCTION FN_GenerateANMShipmentId   
END
GO
CREATE FUNCTION [dbo].[FN_GenerateANMShipmentId]     
(  
	 @SenderId INT  
	 ,@Source CHAR(1)  
...
BEGIN  
	...
	SET @LastShipmentId = (SELECT TOP 1 ShipmentID FROM Tbl_ANMShipment WITH(NOLOCK) WHERE RIID = @SenderId  AND ShipmentID LIKE '%'+@Source     
							AND ShipmentID LIKE @SenderCode +'/'+@MonthYear+'/%' ORDER BY ShipmentID DESC)   
