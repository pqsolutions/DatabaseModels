...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_PathoDiagnosisPendingReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_PathoDiagnosisPendingReport 
END
GO
CREATE PROCEDURE [dbo].[SPC_PathoDiagnosisPendingReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@SubjectType INT
...
BEGIN
	...
	BEGIN
		...
		SELECT * FROM (
			...
			AND HD.[IsConsultSeniorPathologist] = 0
