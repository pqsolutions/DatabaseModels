...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchUpdateRCHIDDetail' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchUpdateRCHIDDetail
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchUpdateRCHIDDetail] 
(
	@ID VARCHAR(100)
)
AS
...
BEGIN
	...
	OldRCHID VARCHAR(200), NewRCHID VARCHAR(200),SCName VARCHAR(250),SCMobileNo VARCHAR(250),DCEmail VARCHAR(500), SCEmail VARCHAR(500))
	...
	BEGIN
		...
		INSERT INTO #TempTable(UniqueSubjectId, SubjectName, ANMName, ANMMobile, ANMEmail, DCName, DCMobile, OldRCHID, NewRCHID,SCName,SCMobileNo,DCEmail,SCEmail)
		SELECT RU.[UniqueSubjectId]
			,SP.[FirstName]
		...
		FROM Tbl_RCHIdUpdationDetails RU
		LEFT JOIN Tbl_SubjectPrimaryDetail SP WITH (NOLOCK) ON SP.[UniqueSubjectID] = RU.[UniqueSubjectID] 
		LEFT JOIN Tbl_UserMaster UM WITH (NOLOCK) ON UM.[ID] = SP.[AssignANM_ID] 
		WHERE RU.ID = @CurrentIndex
	END
