...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchSubjectsMTPPending' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchSubjectsMTPPending
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchSubjectsMTPPending] 
(
	 @DistrictId INT
	 ,@CHCId INT
	 ,@PHCId INT
