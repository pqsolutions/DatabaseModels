...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='TR_AuditTrailForANMDetail' AND [type] = 'tr')
BEGIN
	DROP TRIGGER TR_AuditTrailForANMDetail
END
GO
CREATE TRIGGER TR_AuditTrailForANMDetail ON Tbl_ANMDetail FOR UPDATE,INSERT
AS
    ...
    SELECT @TableName = 'Tbl_ANMDetail'
