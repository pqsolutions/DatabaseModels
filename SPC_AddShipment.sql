...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_AddShipment' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_AddShipment
END
GO
CREATE PROCEDURE [dbo].[SPC_AddShipment]
(	
	@SubjectID INT
...
BEGIN
	BEGIN TRY
		...
		BEGIN
			SELECT @sCount =  count(ID) FROM Tbl_Shipment 
			WHERE SubjectID = @SubjectID AND SampleCollectionID = @SampleCollectionID 
			AND ShipmentType = @ShipmentType AND ShipmentID = @ShipmentID
