...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchMobileChartMTPCompletedCount' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchMobileChartMTPCompletedCount 
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchMobileChartMTPCompletedCount]
(
	@UserId INT
)AS
BEGIN
	...
	BEGIN
		...
		SELECT  @Month,@MonthName,@Year,COUNT(MT.[ID]) AS [TotalCount]
		FROM Tbl_MTPTest MT 
		LEFT JOIN Tbl_SubjectPrimaryDetail SP WITH (NOLOCK) ON SP.[UniqueSubjectID] = MT.[ANWSubjectId]
		WHERE  MONTH(MT.[MTPDateTime]) = @Month AND YEAR(MT.[MTPDateTime]) = @Year
