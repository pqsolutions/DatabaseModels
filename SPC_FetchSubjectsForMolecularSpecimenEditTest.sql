...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchSubjectsForMolecularSpecimenEditTest' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchSubjectsForMolecularSpecimenEditTest
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchSubjectsForMolecularSpecimenEditTest] 
(
	@MolecularLabId INT
)
AS
