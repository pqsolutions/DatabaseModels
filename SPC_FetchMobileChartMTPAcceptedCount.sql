...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchMobileChartMTPAcceptedCount' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchMobileChartMTPAcceptedCount 
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchMobileChartMTPAcceptedCount]
(
	@UserId INT
)AS
BEGIN
	...
	BEGIN
		...
		LEFT JOIN Tbl_PostPNDTScheduling PPS WITH (NOLOCK) ON PPS.[ANWSubjectId] = PPC.[ANWSubjectId]
