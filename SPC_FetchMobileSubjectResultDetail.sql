...
BEGIN
	SELECT SP.[UniqueSubjectID]
		...
		,CASE WHEN (SELECT [dbo].[FN_FindResult](SP.[UniqueSubjectID],'HPLC')) = '' THEN 0 ELSE 1 END AS IsHPLCPositive
	...
	AND (CT.[UpdatedToANM] IS NULL OR ST.[UpdatedToANM] IS NULL OR HT.[UniqueSubjectID] IS NULL)
END