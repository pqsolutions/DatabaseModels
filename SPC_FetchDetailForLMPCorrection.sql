...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchDetailForLMPCorrection' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchDetailForLMPCorrection
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchDetailForLMPCorrection] 
(
	@Input VARCHAR(MAX)
)
