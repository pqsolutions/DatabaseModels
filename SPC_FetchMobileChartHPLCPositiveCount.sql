...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchMobileChartHPLCPositiveCount' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchMobileChartHPLCPositiveCount
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchMobileChartHPLCPositiveCount]
(
	@UserId INT
)AS
BEGIN
	...
	BEGIN
		...
		WHERE  MONTH(PRSD.[HPLCUpdatedOn]) = @Month AND YEAR(PRSD.[HPLCUpdatedOn]) = @Year
