...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchMobileChartSubjectCount' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchMobileChartSubjectCount 
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchMobileChartSubjectCount]
(
	@UserId INT
)AS
BEGIN
	...
	BEGIN
		...
		SELECT  @Month,@MonthName,@Year,COUNT(ID) AS [TotalCount]
		FROM Tbl_SubjectPrimaryDetail 
		WHERE  MONTH(DateofRegister) = @Month AND YEAR(DateofRegister) = @Year AND  AssignANM_ID = @UserId AND ID IN (SELECT SubjectID FROM Tbl_SubjectParentDetail)
