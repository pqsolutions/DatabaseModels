...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchAllSubjectDetailByCHC' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchAllSubjectDetailByCHC
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchAllSubjectDetailByCHC]  
(  
	@UserId INT 
	,@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50) 
...
AS  
DECLARE @CHCID INT , @RegisterdFrom INT , @StartDate VARCHAR(50), @EndDate VARCHAR(50)
BEGIN  
	...
	END
	 ...
	 SELECT @RegisterdFrom = ID FROM Tbl_ConstantValues WHERE CommonName = 'CHC' AND comments = 'RegisterFrom'  
	 ...
	 AND SPAD.[SubjectID] IN (SELECT ID FROM Tbl_SubjectPrimaryDetail WHERE CHCID  = @CHCID AND [RegisteredFrom] = @RegisterdFrom)  
