...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchBarcodeErrorCorrectionReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchBarcodeErrorCorrectionReport --'20-03-2021','25-06-2021', 0,0,0,0
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchBarcodeErrorCorrectionReport] 
(
...
BEGIN
	SELECT  (SP.[FirstName] +' '+ SP.[LastName]) AS SubjectName
		...
		,BUD.[ExistBarcodeNo] AS OldBarcodeNo
		,BUD.[NewBarcodeNo] AS RevisedBarcodeNo
		,CONVERT(VARCHAR,BUD.[CreatedOn],103) AS DateChanged
	...
	FROM [dbo].[Tbl_BarcodeUpdationDetails] BUD  
	LEFT JOIN [dbo].[Tbl_SubjectPrimaryDetail] SP WITH (NOLOCK) ON SP.[UniqueSubjectID] = BUD.[UniqueSubjectID] 
	...
	AND (BUD.[CreatedOn] BETWEEN CONVERT(DATE,@FromDate,103) AND CONVERT(DATE,@ToDate,103))
END