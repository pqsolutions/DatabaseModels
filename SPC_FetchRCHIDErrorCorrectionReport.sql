...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchRCHIDErrorCorrectionReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchRCHIDErrorCorrectionReport --'20-03-2021','25-06-2021', 0,0,0,0
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchRCHIDErrorCorrectionReport] 
(
...
BEGIN
	SELECT  (SP.[FirstName] +' '+ SP.[LastName]) AS SubjectName
		...
		,RUD.[ExistRCHId] AS OldRCHID
		,RUD.[NewRCHId] AS RevisedRCHID
		,CONVERT(VARCHAR,RUD.[CreatedOn],103) AS DateChanged
	...
	FROM [dbo].[Tbl_RCHIdUpdationDetails] RUD  
	LEFT JOIN [dbo].[Tbl_SubjectPrimaryDetail] SP WITH (NOLOCK) ON SP.[UniqueSubjectID] = RUD.[UniqueSubjectID] 
	...
	AND (RUD.[CreatedOn] BETWEEN CONVERT(DATE,@FromDate,103) AND CONVERT(DATE,@ToDate,103))
END