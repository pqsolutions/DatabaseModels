...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchMobileChartSampleCollectionCount' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchMobileChartSampleCollectionCount
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchMobileChartSampleCollectionCount]
(
	@UserId INT
)AS
BEGIN
	...
	BEGIN
		...
		SELECT  @Month,@MonthName,@Year,COUNT(SC.[ID]) AS [TotalCount]
		FROM Tbl_SampleCollection SC 
		LEFT JOIN Tbl_SubjectPrimaryDetail SP WITH (NOLOCK) ON SP.[UniqueSubjectID] = SC.[UniqueSubjectID]
		WHERE  MONTH(SC.[SampleCollectionDate]) =@Month AND YEAR(DateofRegister) = @Year AND   SP.[AssignANM_ID] = @UserId
