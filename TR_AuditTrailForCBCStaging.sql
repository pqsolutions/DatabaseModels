...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='TR_AuditTrailForCBCStaging' AND [type] = 'tr')
BEGIN
	DROP TRIGGER TR_AuditTrailForCBCStaging
END
GO
CREATE TRIGGER TR_AuditTrailForCBCStaging ON Tbl_CBCTestedDetail FOR UPDATE,INSERT
AS
    ...
    SELECT @TableName = 'Tbl_CBCTestedDetail'
