...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='TR_AuditTrailForPNDTReferal' AND [type] = 'tr')
BEGIN
	DROP TRIGGER TR_AuditTrailForPNDTReferal
END
GO
CREATE TRIGGER TR_AuditTrailForPNDTReferal ON Tbl_PrePNDTReferal FOR UPDATE,INSERT
AS
    ...
    SELECT @TableName = 'Tbl_PrePNDTReferal'
