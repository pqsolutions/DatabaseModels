...
--Fetch Subject Details  which are MTP Referal for  ANM in Notification

IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchANMNotificationMTPReferal' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchANMNotificationMTPReferal 
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchANMNotificationMTPReferal] 
(
	@UserId INT
)
AS
BEGIN
	...
	WHERE SP.[AssignANM_ID]  = @UserId   AND MTPR.[IsNotified] = 0 AND MTPR.[IsMTPCompleted] = 0 AND PRSD.[HPLCStatus] = 'P' AND PRSDS.[HPLCStatus] = 'P'
