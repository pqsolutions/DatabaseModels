...
--Fetch Subject Details  which are MTP Service for ANM

IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchANMMTPService' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchANMMTPService 
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchANMMTPService] 
(
	@UserId INT
)
AS
