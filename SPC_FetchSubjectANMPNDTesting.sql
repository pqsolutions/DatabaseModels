...
--Fetch Subject Details  which are PrePNDTesting for ANM 

IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchSubjectANMPNDTesting' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchSubjectANMPNDTesting 
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchSubjectANMPNDTesting] 
(
	@UserId INT
)
AS
