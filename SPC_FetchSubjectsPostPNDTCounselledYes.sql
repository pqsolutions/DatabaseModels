...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchSubjectsPostPNDTCounselledYes' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchSubjectsPostPNDTCounselledYes
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchSubjectsPostPNDTCounselledYes] 
(
	 @UserId INT
	 ,@DistrictId INT
	 ,@CHCId INT
