...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchBarcodeExist' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchBarcodeExist
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchBarcodeExist] 
(
...
BEGIN
	...
	WHERE  SC.[BarcodeNo] = @BarcodeNo ORDER BY SC.ID DESC
