...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_PathoDiagnosisCompletedReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_PathoDiagnosisCompletedReport 
END
GO
CREATE PROCEDURE [dbo].[SPC_PathoDiagnosisCompletedReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@SubjectType INT
