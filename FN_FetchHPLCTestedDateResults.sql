...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='FN_FetchHPLCTestedDateResults' AND [type] = 'FN')
BEGIN
	DROP FUNCTION [dbo].[FN_FetchHPLCTestedDateResults]
END 
GO
CREATE FUNCTION [FN_FetchHPLCTestedDateResults]     
(  
 @Barcode VARCHAR(250)
)   
RETURNS VARCHAR(100)          
...
BEGIN  
	...
	IF NOT EXISTS (SELECT TestedDateTime FROM Tbl_HPLCTestedDetail WHERE Barcode = @Barcode)
	BEGIN
		SET @Result = '--'
	END
	ELSE
