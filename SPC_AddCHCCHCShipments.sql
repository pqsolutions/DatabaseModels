...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_AddCHCCHCShipments' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_AddCHCCHCShipments
END
GO
CREATE PROCEDURE [dbo].[SPC_AddCHCCHCShipments]
(	
...
BEGIN
	BEGIN TRY
		...
		BEGIN
			...
			ELSE
			BEGIN
				SET @GeneratedShipmentID = (SELECT  [dbo].[FN_GenerateANMCHCShipmentId](@CollectionCHCID,@Source,@ShipmentFrom))
