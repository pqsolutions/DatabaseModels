...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='FN_FetchHbFResults' AND [type] = 'FN')
BEGIN
	DROP FUNCTION [dbo].[FN_FetchHbFResults]
END 
GO
CREATE FUNCTION [FN_FetchHbFResults]     
(  
 @Barcode VARCHAR(250)
)   
RETURNS VARCHAR(100)          
...
BEGIN  
	...
	IF NOT EXISTS (SELECT HbF FROM Tbl_HPLCTestedDetail WHERE Barcode = @Barcode)
	BEGIN
		SET @Result = '--'
	END
	ELSE
