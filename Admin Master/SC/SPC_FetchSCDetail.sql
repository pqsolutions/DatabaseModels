...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchSCDetail' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchSCDetail
END
GO
CREATE Procedure [dbo].[SPC_FetchSCDetail] (@Id INT)
AS
BEGIN
	SELECT S.[ID]
		,S.[CHCID]
