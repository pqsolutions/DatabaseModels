...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchCHCDetail' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchCHCDetail
END
GO
CREATE Procedure [dbo].[SPC_FetchCHCDetail] (@Id INT)
AS
BEGIN
	SELECT C.[ID]
		,C.[CHCname]
