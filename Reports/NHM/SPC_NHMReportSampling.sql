...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_NHMReportSampling' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_NHMReportSampling  --'','',0,0,0,0,3
END
GO
CREATE PROCEDURE [dbo].[SPC_NHMReportSampling]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@DistrictId INT
...
BEGIN
	...
	IF @Status = 1 -- All Sampling Status
	BEGIN
	...
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM Tbl_SubjectPrimaryDetail SP
		LEFT JOIN Tbl_SampleCollection SC WITH (NOLOCK) ON SC.UniqueSubjectID = SP.UniqueSubjectID
		LEFT JOIN Tbl_CHCMaster CM  WITH (NOLOCK) ON CM.ID = SP.CHCID
		LEFT JOIN Tbl_BlockMaster BM WITH (NOLOCK) ON CM.BlockID = BM.ID
		LEFT JOIN Tbl_UserMaster UM WITH (NOLOCK) ON UM.ID = SP.AssignANM_ID
		WHERE SP.ChildSubjectTypeID = 1 AND SP.ID IN (SELECT SubjectID FROM Tbl_SubjectParentDetail)
		AND SP.[ID] IN (SELECT SubjectID FROM Tbl_SampleCollection WHERE SampleDamaged != 1 AND SampleTimeoutExpiry != 1)
		AND (SP.DistrictID = @DistrictId OR @DistrictId = 0)
		AND (BM.ID = @BlockId OR @BlockId = 0)
		AND (SP.CHCID = @CHCID OR @CHCID = 0)
		AND (SP.AssignANM_ID = @ANMID OR @ANMID = 0)
