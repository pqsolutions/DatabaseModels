...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_CHCsCHCReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_CHCsCHCReport  --'','',1,0,0,5
END
GO
CREATE PROCEDURE [dbo].[SPC_CHCsCHCReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@CHCID INT
...
BEGIN
	...
	BEGIN
		...
		SELECT * FROM (
			SELECT	UM.[User_gov_code] AS ANMID
				...
				,CASE WHEN CBC.[ID]  IS NULL AND SST.[ID] IS NULL THEN 'Pending' WHEN CBC.[ID]  IS NOT NULL AND SST.[ID] IS NULL THEN CBC.[CBCResult] 
					WHEN CBC.[ID]  IS NULL AND SST.[ID] IS NOT NULL AND SST.[IsPositive] = 1 THEN 'SST Positive'
					WHEN CBC.[ID]  IS NULL AND SST.[ID] IS NOT NULL AND SST.[IsPositive] = 0 THEN 'SST Negative' END AS   TestResult
			...
			AND (ASD.[BarcodeNo] NOT IN (SELECT BarcodeNo FROM Tbl_CBCTestResult) OR ASD.[BarcodeNo] NOT IN (SELECT BarcodeNo FROM Tbl_SSTestResult))
	...
	BEGIN
		...
		SELECT * FROM (
			...
			AND ASD.[IsAccept] = 1 AND ASD.[SampleDamaged] = 0 AND ASD.[SampleTimeoutExpiry] = 0
			AND CBC.[IsPositive] = 1 AND SST.[IsPositive] = 1
		)GROUPS
		WHERE GROUPS.[ROW NUMBER] = 1 
		ORDER BY GROUPS.UniqueSubjectID ASC
	END
	...
	BEGIN
		...
		SELECT * FROM (
			...
			AND ASD.[BarcodeNo] IN (SELECT BarcodeNo FROM Tbl_SSTestResult)
			AND ASD.[IsAccept] = 1 AND ASD.[SampleDamaged] = 0 AND ASD.[SampleTimeoutExpiry] = 0
			AND CBC.[IsPositive] = 0 AND SST.[IsPositive] = 0
		)GROUPS
		WHERE GROUPS.[ROW NUMBER] = 1 
		ORDER BY GROUPS.UniqueSubjectID ASC
	END
