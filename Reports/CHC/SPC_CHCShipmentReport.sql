...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_CHCShipmentReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_CHCShipmentReport -- '','',1,0,0,1
END
GO
CREATE PROCEDURE [dbo].[SPC_CHCShipmentReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@CHCID INT
