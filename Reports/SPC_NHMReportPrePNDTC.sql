...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_NHMReportPrePNDTC' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_NHMReportPrePNDTC  --'','',0,0,0,0,1
END
GO
CREATE PROCEDURE [dbo].[SPC_NHMReportPrePNDTC]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@DistrictId INT
...
BEGIN
	...
	IF @Status = 1 -- All Pre PNDT Counselling (Pending & Completed)
	BEGIN
	...
	END 
	IF @Status = 2  -- Pre PNDT Counselling Pending
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_HPLCDiagnosisResult HD
		LEFT JOIN Tbl_HPLCTestResult HT WITH (NOLOCK) ON HT.BarcodeNo = HD.BarcodeNo
		...
		AND HD.IsDiagnosisComplete = 1 AND HD.IsNormal = 0 AND LEN(SP.SpouseSubjectID) > 0 
		AND SP.SpouseSubjectID IN (SELECT UniqueSubjectID FROM Tbl_HPLCDiagnosisResult WHERE  IsDiagnosisComplete = 1 AND IsNormal = 0)
		AND SP.UniqueSubjectID  IN (SELECT ANWSubjectId FROM Tbl_PrePNDTScheduling WHERE IsCounselled = 0)
	END 
	IF @Status = 3  -- Pre PNDT Counselling Completed. Decision Agreed Yes for PNDT.
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_HPLCDiagnosisResult HD
		LEFT JOIN Tbl_HPLCTestResult HT WITH (NOLOCK) ON HT.BarcodeNo = HD.BarcodeNo
	...
	END 
	IF @Status = 4  -- Pre PNDT Counselling Completed. Decision Agreed No for PNDT.
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_HPLCDiagnosisResult HD
		LEFT JOIN Tbl_HPLCTestResult HT WITH (NOLOCK) ON HT.BarcodeNo = HD.BarcodeNo
		...
		AND SP.SpouseSubjectID IN (SELECT UniqueSubjectID FROM Tbl_HPLCDiagnosisResult WHERE  IsDiagnosisComplete = 1 AND IsNormal = 0)
		AND SP.UniqueSubjectID  IN (SELECT ANWSubjectId FROM Tbl_PrePNDTScheduling WHERE IsCounselled = 1)
		AND SP.UniqueSubjectID  IN (SELECT ANWSubjectId FROM Tbl_PrePNDTCounselling WHERE IsPNDTAgreeNo = 1)
	END 
	IF @Status = 5  -- Pre PNDT Counselling Completed. Decision awaiting for PNDT.
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_HPLCDiagnosisResult HD
		LEFT JOIN Tbl_HPLCTestResult HT WITH (NOLOCK) ON HT.BarcodeNo = HD.BarcodeNo
		...
		AND SP.SpouseSubjectID IN (SELECT UniqueSubjectID FROM Tbl_HPLCDiagnosisResult WHERE  IsDiagnosisComplete = 1 AND IsNormal = 0)
		AND SP.UniqueSubjectID  IN (SELECT ANWSubjectId FROM Tbl_PrePNDTScheduling WHERE IsCounselled = 1)
		AND SP.UniqueSubjectID  IN (SELECT ANWSubjectId FROM Tbl_PrePNDTCounselling WHERE IsPNDTAgreePending = 1)
	END 
