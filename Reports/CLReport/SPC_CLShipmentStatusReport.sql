...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_CLShipmentStatusReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_CLShipmentStatusReport 
END
GO
CREATE PROCEDURE [dbo].[SPC_CLShipmentStatusReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@SubjectType INT
...
BEGIN
	...
	BEGIN
		...
		SELECT * FROM (
			SELECT	UM.[User_gov_code] AS ANMID
				...
				,HT.[HPLCResult] AS TestResult
				,(CONVERT(VARCHAR,CLS.[DateofShipment],103) + ' ' + CONVERT(VARCHAR(5),CLS.[TimeofShipment],108))   AS CLShipmentDateTime
				,CLSD.[ShipmentID] AS CLShipmentId
			...
			LEFT JOIN [dbo].[Tbl_CentralLabShipments] CLS WITH (NOLOCK) ON CLS.[ID] = CLSD.[ShipmentID]
			WHERE  SPRD.[ID] IN (SELECT SubjectID FROM [dbo].[Tbl_SubjectParentDetail])
			AND(HT.[HPLCTestCompletedOn] BETWEEN  CONVERT(DATETIME,@StartDate,103) AND CONVERT(DATETIME,@EndDate,103))
			...
			AND (HT.[CentralLabId] = @CentalLabID)
			...
			AND HT.[IsPositive] = 1 
			AND CSD.[BarcodeNo]  IN (SELECT BarcodeNo FROM [Tbl_CentralLabShipmentsDetail])
		)GROUPS
	...
	BEGIN
		...
		SELECT * FROM (
			...
			AND CSD.[SampleDamaged] = 0 AND CSD.[SampleTimeoutExpiry] = 0
			AND HT.[IsPositive] = 1 
			AND CSD.[BarcodeNo]  NOT IN (SELECT BarcodeNo FROM [Tbl_CentralLabShipmentsDetail])
		)GROUPS
		WHERE GROUPS.[ROW NUMBER] = 1 
		ORDER BY GROUPS.UniqueSubjectID ASC
	END
