...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_CLSampleRecpReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_CLSampleRecpReport 
END
GO
CREATE PROCEDURE [dbo].[SPC_CLSampleRecpReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@SubjectType INT
	...
	BEGIN
		...
		SELECT * FROM (
			SELECT	UM.[User_gov_code] AS ANMID
				...
				,CASE WHEN CSD.[SampleDamaged] = 1 THEN 'Sample Damaged' WHEN CSD.[SampleTimeoutExpiry] = 1 THEN 'Sample Timeout' ELSE 'Normal' END TimeoutDamaged
