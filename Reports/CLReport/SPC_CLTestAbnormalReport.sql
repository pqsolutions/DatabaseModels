...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_CLTestAbnormalReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_CLTestAbnormalReport 
END
GO
CREATE PROCEDURE [dbo].[SPC_CLTestAbnormalReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@SubjectType INT
...
BEGIN
	...
	BEGIN
		...
		SELECT * FROM (
			...
			AND SC.[BarcodeNo]  IN (SELECT BarcodeNo FROM Tbl_HPLCTestResult)
