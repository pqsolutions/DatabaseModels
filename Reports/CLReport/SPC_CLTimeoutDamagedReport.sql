...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_CLTimeoutDamagedReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_CLTimeoutDamagedReport 
END
GO
CREATE PROCEDURE [dbo].[SPC_CLTimeoutDamagedReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@SubjectType INT
...
BEGIN
	...
	BEGIN
		...
		SELECT * FROM (
			...
			AND (SPRD.[AssignANM_ID] = @ANMID OR @ANMID = 0)
				AND SC.[IsAccept] = 0 AND CSD.[SampleTimeoutExpiry] = 1 AND SC.[RejectAt] = 'Central Lab Receipt'
		)GROUPS
	...
	BEGIN
		...
		SELECT * FROM (
			...
			AND (SPRD.[PHCID] = @PHCID OR @PHCID = 0)
			AND (SPRD.[AssignANM_ID] = @ANMID OR @ANMID = 0)
			AND SC.[IsAccept] = 0 AND CSD.[SampleDamaged] = 1 AND SC.[RejectAt] = 'Central Lab Receipt'
		)GROUPS
		WHERE GROUPS.[ROW NUMBER] = 1 
		ORDER BY GROUPS.UniqueSubjectID ASC
	END
