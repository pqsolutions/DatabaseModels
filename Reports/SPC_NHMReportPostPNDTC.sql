...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_NHMReportPostPNDTC' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_NHMReportPostPNDTC  --'','',0,0,0,0,1
END
GO
CREATE PROCEDURE [dbo].[SPC_NHMReportPostPNDTC]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@DistrictId INT
...
BEGIN
	...
	IF @Status = 1 -- All Post PNDT Counselling (Pending & Completed)
	BEGIN
	...
	END 
	IF @Status = 2  -- Post PNDT Counselling Pending
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_HPLCDiagnosisResult HD
		LEFT JOIN Tbl_HPLCTestResult HT WITH (NOLOCK) ON HT.BarcodeNo = HD.BarcodeNo
		...
		AND SP.UniqueSubjectID IN (SELECT ANWSubjectId FROM Tbl_PrePNDTCounselling WHERE IsPNDTAgreeYes = 1)
		AND SP.UniqueSubjectID IN (SELECT ANWSubjectId FROM Tbl_PNDTest WHERE IsCompletePNDT=1)
		AND SP.UniqueSubjectID IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled = 0)
	END 
	IF @Status = 3  -- Post PNDT Counselling Completed. Decision Agreed Yes for MTP.
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_HPLCDiagnosisResult HD
		LEFT JOIN Tbl_HPLCTestResult HT WITH (NOLOCK) ON HT.BarcodeNo = HD.BarcodeNo
		...
		AND SP.UniqueSubjectID  IN (SELECT ANWSubjectId FROM Tbl_PrePNDTCounselling WHERE IsPNDTAgreeYes = 1)
		AND SP.UniqueSubjectID IN (SELECT ANWSubjectId FROM Tbl_PNDTest WHERE IsCompletePNDT=1)
		AND SP.UniqueSubjectID IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled = 1)
		AND SP.UniqueSubjectID  IN (SELECT ANWSubjectId FROM Tbl_PostPNDTCounselling WHERE IsMTPTestdAgreedYes = 1)
	END 
	IF @Status = 4  -- Post PNDT Counselling Completed. Decision Agreed No for MTP.
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_HPLCDiagnosisResult HD
		LEFT JOIN Tbl_HPLCTestResult HT WITH (NOLOCK) ON HT.BarcodeNo = HD.BarcodeNo
		...
		AND SP.UniqueSubjectID IN (SELECT ANWSubjectId FROM Tbl_PNDTest WHERE IsCompletePNDT=1)
		AND SP.UniqueSubjectID IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled = 1)
		AND SP.UniqueSubjectID  IN (SELECT ANWSubjectId FROM Tbl_PostPNDTCounselling WHERE IsMTPTestdAgreedNo = 1)
	END 
	IF @Status = 5  -- Post PNDT Counselling Completed. Decision awaiting for PNDT.
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_HPLCDiagnosisResult HD
		LEFT JOIN Tbl_HPLCTestResult HT WITH (NOLOCK) ON HT.BarcodeNo = HD.BarcodeNo
		...
		AND SP.UniqueSubjectID IN (SELECT ANWSubjectId FROM Tbl_PNDTest WHERE IsCompletePNDT=1)
		AND SP.UniqueSubjectID IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled = 1)
		AND SP.UniqueSubjectID  IN (SELECT ANWSubjectId FROM Tbl_PostPNDTCounselling WHERE IsMTPTestdAgreedPending = 1)
	END 
