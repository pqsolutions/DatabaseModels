...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_NHMReportMTP' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_NHMReportMTP  --'','',0,0,0,0,1
END
GO
CREATE PROCEDURE [dbo].[SPC_NHMReportMTP]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@DistrictId INT
...
BEGIN
	...
	IF @Status = 1 -- All  MTP Details (Pending & Completed)
	BEGIN
	...
	BEGIN
		...
		AND SP.UniqueSubjectID IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled = 1)
		AND SP.UniqueSubjectID  IN (SELECT ANWSubjectId FROM Tbl_PostPNDTCounselling WHERE IsMTPTestdAgreedYes = 1)
		AND SP.UniqueSubjectID NOT IN (SELECT ANWSubjectId FROM Tbl_MTPTest)
	END 
	IF @Status = 3   -- MTP Completed
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_HPLCDiagnosisResult HD
		LEFT JOIN Tbl_HPLCTestResult HT WITH (NOLOCK) ON HT.BarcodeNo = HD.BarcodeNo
		...
		AND SP.UniqueSubjectID IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled = 1)
		AND SP.UniqueSubjectID  IN (SELECT ANWSubjectId FROM Tbl_PostPNDTCounselling WHERE IsMTPTestdAgreedYes = 1)
		AND SP.UniqueSubjectID IN (SELECT ANWSubjectId FROM Tbl_MTPTest)
	END 
