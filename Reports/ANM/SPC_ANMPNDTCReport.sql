...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_ANMPNDTCReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_ANMPNDTCReport  --'','',2,0,1,5
END
GO
CREATE PROCEDURE [dbo].[SPC_ANMPNDTCReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@ANMID INT
...
BEGIN
	...
	BEGIN
		...
		SELECT * FROM (
			...
			AND SPRD.[UniqueSubjectID] NOT IN (SELECT ANWSubjectId FROM Tbl_PrePNDTScheduling)
			AND SPRD.[UniqueSubjectID] NOT IN (SELECT SpouseSubjectId FROM Tbl_PrePNDTScheduling)
		)GROUPS
	...
	BEGIN
		...
		SELECT * FROM (
			SELECT	UM.[User_gov_code] AS ANMID
				...
				,CASE WHEN CCS.[GenratedShipmentID] IS NULL THEN  '--' ELSE (CONVERT(VARCHAR,CCS.[DateofShipment],103) + ' ' +CONVERT(VARCHAR(5),CCS.[TimeofShipment],108)) END AS CHCShipmentDateTime
				,HT.[LabDiagnosis] AS HPLCPathoDiagnosis
				,('Sch Dt: ' +  (CONVERT(VARCHAR,PPS.[CounsellingDateTime],103) + ' ' +CONVERT(VARCHAR(5),PPS.[CounsellingDateTime],108))) AS PNDT
				,'--' AS MTP
				,(SELECT  [dbo].[FN_FindSubjectStage](SPRD.[UniqueSubjectID])) AS CurrentStatus
				,ROW_NUMBER() OVER (PARTITION BY SPRD.[UniqueSubjectID]  ORDER BY SC.[CreatedOn] DESC) AS [ROW NUMBER]
			FROM [dbo].[Tbl_SubjectPrimaryDetail] AS SPRD
			...
			AND LEN(SPRD.[SpouseSubjectID]) > 0
			AND SPRD.[SpouseSubjectID] IN (SELECT UniqueSubjectID FROM Tbl_HPLCDiagnosisResult WHERE IsDiagnosisComplete = 1 AND IsNormal = 0)
			AND (SPRD.[UniqueSubjectID] IN (SELECT ANWSubjectId FROM Tbl_PrePNDTScheduling WHERE IsCounselled=0) 
			OR SPRD.[UniqueSubjectID] IN (SELECT SpouseSubjectId FROM Tbl_PrePNDTScheduling WHERE IsCounselled=0))
		)GROUPS
		WHERE GROUPS.[ROW NUMBER] = 1 
		ORDER BY GROUPS.UniqueSubjectID ASC
	END
	...
	BEGIN
		...
		SELECT * FROM (
			SELECT	UM.[User_gov_code] AS ANMID
				...
				,CASE WHEN CCS.[GenratedShipmentID] IS NULL THEN  '--' ELSE (CONVERT(VARCHAR,CCS.[DateofShipment],103) + ' ' +CONVERT(VARCHAR(5),CCS.[TimeofShipment],108)) END AS CHCShipmentDateTime
				,HT.[LabDiagnosis] AS HPLCPathoDiagnosis
				,('Sch Dt: ' +  (CONVERT(VARCHAR,PPS.[CounsellingDateTime],103) + ' ' +CONVERT(VARCHAR(5),PPS.[CounsellingDateTime],108)) +', Disagreed') AS PNDT
				,'--' AS MTP
				,(SELECT  [dbo].[FN_FindSubjectStage](SPRD.[UniqueSubjectID])) AS CurrentStatus
				,ROW_NUMBER() OVER (PARTITION BY SPRD.[UniqueSubjectID]  ORDER BY SC.[CreatedOn] DESC) AS [ROW NUMBER]
			FROM [dbo].[Tbl_SubjectPrimaryDetail] AS SPRD
			...
			AND (SPRD.[UniqueSubjectID] IN (SELECT ANWSubjectId FROM Tbl_PrePNDTScheduling WHERE IsCounselled=1) 
			OR SPRD.[UniqueSubjectID] IN (SELECT SpouseSubjectId FROM Tbl_PrePNDTScheduling WHERE IsCounselled=1))
			AND (SPRD.[UniqueSubjectID] IN (SELECT ANWSubjectId FROM Tbl_PrePNDTCounselling WHERE IsPNDTAgreeNo=1) 
			OR SPRD.[UniqueSubjectID] IN (SELECT SpouseSubjectId FROM Tbl_PrePNDTCounselling WHERE IsPNDTAgreeNo=1))
		)GROUPS
		WHERE GROUPS.[ROW NUMBER] = 1 
		ORDER BY GROUPS.UniqueSubjectID ASC
	END
	...
	BEGIN
		...
		SELECT * FROM (
			SELECT	UM.[User_gov_code] AS ANMID
				...
				,CASE WHEN CCS.[GenratedShipmentID] IS NULL THEN  '--' ELSE (CONVERT(VARCHAR,CCS.[DateofShipment],103) + ' ' +CONVERT(VARCHAR(5),CCS.[TimeofShipment],108)) END AS CHCShipmentDateTime
				,HT.[LabDiagnosis] AS HPLCPathoDiagnosis
				,('Sch Dt: ' +  (CONVERT(VARCHAR,PPS.[CounsellingDateTime],103) + ' ' +CONVERT(VARCHAR(5),PPS.[CounsellingDateTime],108)) +', Dec Awaited') AS PNDT
				,'--' AS MTP
				,(SELECT  [dbo].[FN_FindSubjectStage](SPRD.[UniqueSubjectID])) AS CurrentStatus
				,ROW_NUMBER() OVER (PARTITION BY SPRD.[UniqueSubjectID]  ORDER BY SC.[CreatedOn] DESC) AS [ROW NUMBER]
			FROM [dbo].[Tbl_SubjectPrimaryDetail] AS SPRD
			...
			AND (SPRD.[UniqueSubjectID] IN (SELECT ANWSubjectId FROM Tbl_PrePNDTScheduling WHERE IsCounselled=1) 
			OR SPRD.[UniqueSubjectID] IN (SELECT SpouseSubjectId FROM Tbl_PrePNDTScheduling WHERE IsCounselled=1))
			AND (SPRD.[UniqueSubjectID] IN (SELECT ANWSubjectId FROM Tbl_PrePNDTCounselling WHERE IsPNDTAgreePending=1) 
			OR SPRD.[UniqueSubjectID] IN (SELECT SpouseSubjectId FROM Tbl_PrePNDTCounselling WHERE IsPNDTAgreePending=1))
		)GROUPS
		WHERE GROUPS.[ROW NUMBER] = 1 
		ORDER BY GROUPS.UniqueSubjectID ASC
	END
