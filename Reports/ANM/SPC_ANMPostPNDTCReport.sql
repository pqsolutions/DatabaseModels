...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_ANMPostPNDTCReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_ANMPostPNDTCReport -- '','',2,0,1,1
END
GO
CREATE PROCEDURE [dbo].[SPC_ANMPostPNDTCReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@ANMID INT
...
BEGIN
	...
	BEGIN
		...
		SELECT * FROM (
			...
			AND SPRD.[UniqueSubjectID] NOT IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling)
			AND SPRD.[UniqueSubjectID] NOT IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling)
		)GROUPS
	...
	BEGIN
		...
		SELECT * FROM (
			SELECT	UM.[User_gov_code] AS ANMID
				...
				,('Sch Dt: ' +  (CONVERT(VARCHAR,PPS.[CounsellingDateTime],103) + ' ' +CONVERT(VARCHAR(5),PPS.[CounsellingDateTime],108)) +', Agreed, PNDT Dt: ' +
				(CONVERT(VARCHAR,PT.[PNDTDateTime],103) + ' ' +CONVERT(VARCHAR(5),PT.[PNDTDateTime],108)) +', ' + ISNULL(PTR.[ResultName],'')) AS PNDT
				,('Sch Dt: ' +  (CONVERT(VARCHAR,PoPS.[CounsellingDateTime],103) + ' ' +CONVERT(VARCHAR(5),PoPS.[CounsellingDateTime],108))) AS MTP
				,(SELECT  [dbo].[FN_FindSubjectStage](SPRD.[UniqueSubjectID])) AS CurrentStatus
				,ROW_NUMBER() OVER (PARTITION BY SPRD.[UniqueSubjectID]  ORDER BY SC.[CreatedOn] DESC) AS [ROW NUMBER]
			FROM [dbo].[Tbl_SubjectPrimaryDetail] AS SPRD
			LEFT JOIN [dbo].[Tbl_SubjectPregnancyDetail] SPD WITH (NOLOCK) ON SPD.[UniqueSubjectID] = SPRD.[UniqueSubjectID] 
			...
			LEFT JOIN [dbo].[Tbl_PNDTest] PT WITH (NOLOCK) ON PT.[ANWSubjectId] = HD.[UniqueSubjectID] OR PT.[SpouseSubjectId] = HD.[UniqueSubjectID]
			LEFT JOIN [dbo].[Tbl_PNDTResultMaster] PTR WITH (NOLOCK) ON PT.[PNDTResultId] = PTR.[ID]
			LEFT JOIN [dbo].[Tbl_PostPNDTScheduling] PoPS WITH (NOLOCK) ON PoPS.[ANWSubjectId] = HD.[UniqueSubjectID] OR PoPS.[SpouseSubjectId] = HD.[UniqueSubjectID]
			WHERE  SPRD.[ID] IN (SELECT SubjectID FROM [dbo].[Tbl_SubjectParentDetail])
			AND(SPRD.[DateofRegister] BETWEEN  CONVERT(DATETIME,@StartDate,103) AND CONVERT(DATETIME,@EndDate,103))
			AND (SPRD.[RIID] = @RIID OR @RIID = 0)
			AND (SPRD.[ChildSubjectTypeID] = @SubjectType OR @SubjectType = 0)
			...
			AND (SPRD.[UniqueSubjectID]  IN (SELECT ANWSubjectId FROM Tbl_PNDTest WHERE PNDTResultId=2 AND IsCompletePNDT=1) 
			OR SPRD.[UniqueSubjectID] IN (SELECT SpouseSubjectId FROM Tbl_PNDTest WHERE PNDTResultId=2 AND IsCompletePNDT=1))
			AND SPRD.[UniqueSubjectID]  IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled=0)
			AND SPRD.[UniqueSubjectID]  IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled=0)
		)GROUPS
		WHERE GROUPS.[ROW NUMBER] = 1 
		ORDER BY GROUPS.UniqueSubjectID ASC
	...
	BEGIN
		...
		SELECT * FROM (
			...
			LEFT JOIN [dbo].[Tbl_PNDTResultMaster] PTR WITH (NOLOCK) ON PT.[PNDTResultId] = PTR.[ID]
			LEFT JOIN [dbo].[Tbl_PostPNDTScheduling] PoPS WITH (NOLOCK) ON PoPS.[ANWSubjectId] = HD.[UniqueSubjectID] OR PoPS.[SpouseSubjectId] = HD.[UniqueSubjectID]
			LEFT JOIN [dbo].[Tbl_PostPNDTCounselling] PoPC WITH (NOLOCK) ON PoPC.[ANWSubjectId] = HD.[UniqueSubjectID] OR PoPC.[SpouseSubjectId] = HD.[UniqueSubjectID]
			WHERE  SPRD.[ID] IN (SELECT SubjectID FROM [dbo].[Tbl_SubjectParentDetail])
			AND(SPRD.[DateofRegister] BETWEEN  CONVERT(DATETIME,@StartDate,103) AND CONVERT(DATETIME,@EndDate,103))
			AND (SPRD.[RIID] = @RIID OR @RIID = 0)
			AND (SPRD.[ChildSubjectTypeID] = @SubjectType OR @SubjectType = 0)
			...
			AND (SPRD.[UniqueSubjectID]  IN (SELECT ANWSubjectId FROM Tbl_PNDTest WHERE PNDTResultId=2 AND IsCompletePNDT=1) 
			OR SPRD.[UniqueSubjectID] IN (SELECT SpouseSubjectId FROM Tbl_PNDTest WHERE PNDTResultId=2 AND IsCompletePNDT=1))
			AND SPRD.[UniqueSubjectID]  IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled=1)
			AND SPRD.[UniqueSubjectID]  IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled=1)
			AND (SPRD.[UniqueSubjectID] IN (SELECT ANWSubjectId FROM Tbl_PostPNDTCounselling WHERE IsMTPTestdAgreedYes=1) 
			OR SPRD.[UniqueSubjectID] IN (SELECT SpouseSubjectId FROM Tbl_PostPNDTCounselling WHERE IsMTPTestdAgreedYes=1))
		)GROUPS
		WHERE GROUPS.[ROW NUMBER] = 1 
		ORDER BY GROUPS.UniqueSubjectID ASC
	END
	...
	BEGIN
		...
		SELECT * FROM (
			SELECT	UM.[User_gov_code] AS ANMID
				...
				,('Sch Dt: ' +  (CONVERT(VARCHAR,PPS.[CounsellingDateTime],103) + ' ' +CONVERT(VARCHAR(5),PPS.[CounsellingDateTime],108)) +', Agreed, PNDT Dt: ' +
				(CONVERT(VARCHAR,PT.[PNDTDateTime],103) + ' ' +CONVERT(VARCHAR(5),PT.[PNDTDateTime],108)) +', ' + ISNULL(PTR.[ResultName],'')) AS PNDT
				,('Sch Dt: ' +  (CONVERT(VARCHAR,PoPS.[CounsellingDateTime],103) + ' ' +CONVERT(VARCHAR(5),PoPS.[CounsellingDateTime],108))+', Disagreed') AS MTP
				,(SELECT  [dbo].[FN_FindSubjectStage](SPRD.[UniqueSubjectID])) AS CurrentStatus
				,ROW_NUMBER() OVER (PARTITION BY SPRD.[UniqueSubjectID]  ORDER BY SC.[CreatedOn] DESC) AS [ROW NUMBER]
			FROM [dbo].[Tbl_SubjectPrimaryDetail] AS SPRD
			LEFT JOIN [dbo].[Tbl_SubjectPregnancyDetail] SPD WITH (NOLOCK) ON SPD.[UniqueSubjectID] = SPRD.[UniqueSubjectID] 
			...
			AND SPRD.[UniqueSubjectID]  IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled=1)
			AND SPRD.[UniqueSubjectID]  IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled=1)
			AND (SPRD.[UniqueSubjectID] IN (SELECT ANWSubjectId FROM Tbl_PostPNDTCounselling WHERE IsMTPTestdAgreedNo=1) 
			OR SPRD.[UniqueSubjectID] IN (SELECT SpouseSubjectId FROM Tbl_PostPNDTCounselling WHERE IsMTPTestdAgreedNo=1))
		)GROUPS
		WHERE GROUPS.[ROW NUMBER] = 1 
		ORDER BY GROUPS.UniqueSubjectID ASC
	END
	...
	BEGIN
		...
		SELECT * FROM (
			SELECT	UM.[User_gov_code] AS ANMID
				...
				,('Sch Dt: ' +  (CONVERT(VARCHAR,PPS.[CounsellingDateTime],103) + ' ' +CONVERT(VARCHAR(5),PPS.[CounsellingDateTime],108)) +', Agreed, PNDT Dt: ' +
				(CONVERT(VARCHAR,PT.[PNDTDateTime],103) + ' ' +CONVERT(VARCHAR(5),PT.[PNDTDateTime],108)) +', ' + ISNULL(PTR.[ResultName],'')) AS PNDT
				,('Sch Dt: ' +  (CONVERT(VARCHAR,PoPS.[CounsellingDateTime],103) + ' ' +CONVERT(VARCHAR(5),PoPS.[CounsellingDateTime],108))+',  Dec Awaited') AS MTP
				,(SELECT  [dbo].[FN_FindSubjectStage](SPRD.[UniqueSubjectID])) AS CurrentStatus
				,ROW_NUMBER() OVER (PARTITION BY SPRD.[UniqueSubjectID]  ORDER BY SC.[CreatedOn] DESC) AS [ROW NUMBER]
			FROM [dbo].[Tbl_SubjectPrimaryDetail] AS SPRD
			LEFT JOIN [dbo].[Tbl_SubjectPregnancyDetail] SPD WITH (NOLOCK) ON SPD.[UniqueSubjectID] = SPRD.[UniqueSubjectID] 
			...
			AND SPRD.[UniqueSubjectID]  IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled=1)
			AND SPRD.[UniqueSubjectID]  IN (SELECT ANWSubjectId FROM Tbl_PostPNDTScheduling WHERE IsCounselled=1)
			AND (SPRD.[UniqueSubjectID] IN (SELECT ANWSubjectId FROM Tbl_PostPNDTCounselling WHERE IsMTPTestdAgreedPending=1) 
			OR SPRD.[UniqueSubjectID] IN (SELECT SpouseSubjectId FROM Tbl_PostPNDTCounselling WHERE IsMTPTestdAgreedPending=1))
		)GROUPS
		WHERE GROUPS.[ROW NUMBER] = 1 
		ORDER BY GROUPS.UniqueSubjectID ASC
	END
