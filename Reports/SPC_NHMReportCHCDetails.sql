...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_NHMReportCHCDetails' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_NHMReportCHCDetails  --'','',0,0,0,0,3
END
GO
CREATE PROCEDURE [dbo].[SPC_NHMReportCHCDetails]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@DistrictId INT
...
BEGIN
	...
	IF @Status = 1  -- All CBC Results include cbc positive and cbc negative
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_CBCTestResult CT
		LEFT JOIN Tbl_SampleCollection SC WITH (NOLOCK) ON SC.BarcodeNo = CT.BarcodeNo
		...
		AND CT.[BarcodeNo] IN (SELECT BarcodeNo FROM Tbl_SampleCollection WHERE SampleDamaged != 1 AND SampleTimeoutExpiry != 1)
	...
	END 
	IF @Status = 2  --CBC Positive Results
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_CBCTestResult CT
		LEFT JOIN Tbl_SampleCollection SC WITH (NOLOCK) ON SC.BarcodeNo = CT.BarcodeNo
		...
		AND CT.[IsPositive] = 1
	END 
	IF @Status = 3  --CBC Negative Results
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_CBCTestResult CT
		LEFT JOIN Tbl_SampleCollection SC WITH (NOLOCK) ON SC.BarcodeNo = CT.BarcodeNo
		...
		AND CT.[IsPositive] = 0
	END 
	IF @Status = 4  -- All SST Results include sst positive and sst negative
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_SSTestResult SST
		LEFT JOIN Tbl_SampleCollection SC WITH (NOLOCK) ON SC.BarcodeNo = SST.BarcodeNo
		LEFT JOIN Tbl_SubjectPrimaryDetail SP WITH (NOLOCK) ON SP.UniqueSubjectID = SC.UniqueSubjectID
		LEFT JOIN Tbl_CHCMaster CM  WITH (NOLOCK) ON CM.ID = SP.CHCID
		LEFT JOIN Tbl_BlockMaster BM WITH (NOLOCK) ON CM.BlockID = BM.ID
		LEFT JOIN Tbl_UserMaster UM WITH (NOLOCK) ON UM.ID = SP.AssignANM_ID
		WHERE SP.ChildSubjectTypeID = 1 AND SP.ID IN (SELECT SubjectID FROM Tbl_SubjectParentDetail)
		AND SST.[BarcodeNo] IN (SELECT BarcodeNo FROM Tbl_SampleCollection WHERE SampleDamaged != 1 AND SampleTimeoutExpiry != 1)
		AND (SP.DistrictID = @DistrictId OR @DistrictId = 0)
		AND (BM.ID = @BlockId OR @BlockId = 0)
		AND (SP.CHCID = @CHCID OR @CHCID = 0)
		AND (SP.AssignANM_ID = @ANMID OR @ANMID = 0)
		...
		AND UM.UserType_ID IN (SELECT ID FROM Tbl_UserTypeMaster WHERE Usertype = 'ANM')
	END 
	IF @Status = 5  --SST Positive Results
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_SSTestResult SST
		LEFT JOIN Tbl_SampleCollection SC WITH (NOLOCK) ON SC.BarcodeNo = SST.BarcodeNo
		...
		AND SP.DateofRegister BETWEEN CONVERT(DATE,@StartDate,103) AND CONVERT(DATE,@EndDate,103) 
		AND UM.UserType_ID IN (SELECT ID FROM Tbl_UserTypeMaster WHERE Usertype = 'ANM')
		AND SST.[IsPositive] = 1
	END 
	IF @Status = 6  --SST Negative Results
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_SSTestResult SST
		LEFT JOIN Tbl_SampleCollection SC WITH (NOLOCK) ON SC.BarcodeNo = SST.BarcodeNo
