...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_NHMReportSpouseRegistraion' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_NHMReportSpouseRegistraion -- '','',0,0,0,0,1
END
GO
CREATE PROCEDURE [dbo].[SPC_NHMReportSpouseRegistraion]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@DistrictId INT
...
BEGIN
	...
	--IF @Status = 1  -- All Positive ANW Subjects who registered and not registered spouse
	--BEGIN
	...
	--END 
	IF @Status = 1  --All Positive ANW Subjects who registered spouse
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_HPLCDiagnosisResult HD
		LEFT JOIN Tbl_HPLCTestResult HT WITH (NOLOCK) ON HT.BarcodeNo = HD.BarcodeNo
	...
	END 
	IF @Status = 2  --All Positive ANW Subjects who not registered spouse
	BEGIN
		INSERT INTO #TempUniqueSubjectID([UniqueSubjectId])
		SELECT SP.[UniqueSubjectId] FROM  Tbl_HPLCDiagnosisResult HD
		LEFT JOIN Tbl_HPLCTestResult HT WITH (NOLOCK) ON HT.BarcodeNo = HD.BarcodeNo
