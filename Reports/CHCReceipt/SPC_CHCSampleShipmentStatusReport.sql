...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_CHCSampleShipmentStatusReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_CHCSampleShipmentStatusReport 
END
GO
CREATE PROCEDURE [dbo].[SPC_CHCSampleShipmentStatusReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@CHCID INT
...
BEGIN
	...
	ShipmentDateTime VARCHAR(250), ShipmentId VARCHAR(250), ShipmentReceivedDate VARCHAR(200),  TestDateTime VARCHAR(MAX),
	 TestResult VARCHAR(MAX), CHCShipmentDateTime VARCHAR(250),CHCShipmentId VARCHAR(250), [ROW NUMBER] INT) 
	...
	BEGIN
		...
		SELECT * FROM (
			SELECT	UM.[User_gov_code] AS ANMID
				...
				, ('CBC: '+  CONVERT(VARCHAR,CBC.[TestCompleteOn],103)+  ' '+CONVERT(VARCHAR,CBC.[TestCompleteOn],108) +' ,  SST: '+  CONVERT(VARCHAR,SST.[CreatedOn],103)+  ' '+CONVERT(VARCHAR,SST.[CreatedOn],108)) AS TestDateTime
				,CASE WHEN SST.[IsPositive] = 1 THEN  (CBC.[CBCResult] + ', SST Positive') ELSE (CBC.[CBCResult] + ', SST Negative') END TestResult
				,(CONVERT(VARCHAR,CCS.[DateofShipment],103) + ' ' + CONVERT(VARCHAR(5),CCS.[TimeofShipment],108))   AS CHCShipmentDateTime
				,CSD.[ShipmentID] AS CHCShipmentId
	...
	BEGIN
		...
		SELECT * FROM (
			...
			AND (SPRD.[PHCID] = @PHCID OR @PHCID = 0)
			AND (SPRD.[AssignANM_ID] = @ANMID OR @ANMID = 0)
			AND (CBC.[IsPositive] = 1 OR SST.[IsPositive] = 1)
			AND ASD.[SampleDamaged] = 0 AND ASD.[SampleTimeoutExpiry] = 0
			AND ASD.[BarcodeNo] NOT IN (SELECT BarcodeNo FROM [Tbl_CHCShipmentsDetail])
		)GROUPS
		WHERE GROUPS.[ROW NUMBER] = 1 
		ORDER BY GROUPS.UniqueSubjectID ASC
	END
