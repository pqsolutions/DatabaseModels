...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_CHCSampleSSTPendingReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_CHCSampleSSTPendingReport 
END
GO
CREATE PROCEDURE [dbo].[SPC_CHCSampleSSTPendingReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@CHCID INT
...
BEGIN
	...
	BEGIN
		...
		SELECT * FROM (
			...
			AND SC.[BarcodeNo] NOT IN (SELECT BarcodeNo FROM Tbl_SSTestResult)
