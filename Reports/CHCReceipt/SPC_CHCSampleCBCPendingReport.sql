...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_CHCSampleCBCPendingReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_CHCSampleCBCPendingReport 
END
GO
CREATE PROCEDURE [dbo].[SPC_CHCSampleCBCPendingReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@CHCID INT
	...
	BEGIN
		...
		SELECT * FROM (
			SELECT	UM.[User_gov_code] AS ANMID
				...
				,CASE  WHEN ISNULL(SST.[ID],'') = '' THEN ('CBC: --, SST: --')
				ELSE ('CBC: --, SST: '+  CONVERT(VARCHAR,SST.[CreatedOn],103)+  ' '+CONVERT(VARCHAR,SST.[CreatedOn],108)) END AS TestDateTime
				,CASE  WHEN ISNULL(SST.[ID],'') = '' THEN 'CBC Pending, SST Pending' 
				WHEN SST.[IsPositive] = 1  THEN 'CBC Pending, SST Positive'
				ELSE 'CBC Pending, SST Negative' END AS TestResult
			...
			AND SC.[BarcodeNo] NOT IN (SELECT BarcodeNo FROM Tbl_CBCTestResult)
