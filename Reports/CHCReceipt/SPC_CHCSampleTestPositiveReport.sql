...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_CHCSampleTestPositiveReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_CHCSampleTestPositiveReport 
END
GO
CREATE PROCEDURE [dbo].[SPC_CHCSampleTestPositiveReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@CHCID INT
...
BEGIN
	...
	BEGIN
		...
		SELECT * FROM (
			...
			AND (SST.[IsPositive] = 1 OR CBC.[IsPositive] = 1)
