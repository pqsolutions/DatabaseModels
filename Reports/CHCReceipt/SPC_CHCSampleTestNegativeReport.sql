...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_CHCSampleTestNegativeReport' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_CHCSampleTestNegativeReport 
END
GO
CREATE PROCEDURE [dbo].[SPC_CHCSampleTestNegativeReport]
(
	@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
	,@CHCID INT
	...
	BEGIN
		...
		SELECT * FROM (
			SELECT	UM.[User_gov_code] AS ANMID
				...
				,(CBC.[CBCResult] + ', SST Negative') AS TestResult
