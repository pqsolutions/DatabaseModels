...
--Fetch Subject Details  which are PNDT Referal for ANM in Notification

IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchANMNotificationPNDTReferal' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchANMNotificationPNDTReferal 
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchANMNotificationPNDTReferal] 
(
	@UserId INT
)
AS
BEGIN
	...
	WHERE SP.[AssignANM_ID]  = @UserId   AND PPR.[IsNotified]  = 0 AND PPR.[IsPNDTCompleted] = 0 AND PRSD.[HPLCStatus] = 'P' AND PRSDS.[HPLCStatus] = 'P'
