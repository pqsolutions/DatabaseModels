...
--Fetch Subject Details  which are Damaged Sample particular ANM in Tab Notification

IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchMobileNotificationDamaged' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchMobileNotificationDamaged 
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchMobileNotificationDamaged] 
(
	@ANMID INT
)
AS
BEGIN
	...
	WHERE SC.[CollectedBy] = @ANMID  AND SC.[SampleDamaged] = 1 AND SC.[IsRecollected] != 'Y' AND SP.[IsActive] = 1
