...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchSubjectsPostPNDTCounselledPending' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchSubjectsPostPNDTCounselledPending
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchSubjectsPostPNDTCounselledPending] 
(
	 @UserId INT
	 ,@DistrictId INT
	 ,@CHCId INT
...
BEGIN
	SELECT PT.[ANWSubjectId]
		...
		,(CONVERT(VARCHAR,PPS.[CounsellingDateTime],103)+ ' ' +CONVERT(VARCHAR(5),PPS.[CounsellingDateTime],108)) AS CounsellingDateTime
	...
	AND PPCC.[ANWSubjectId] NOT IN(SELECT ANWSubjectId FROM Tbl_MTPTest) AND PPCC.[IsMTPTestdAgreedPending] = 1 AND PPCC.[IsActive] = 1
