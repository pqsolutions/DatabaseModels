...
--Fetch Subject PNDT and MTP Details For CHC Registration Subject Profile

IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_CHCSubjectProfilePNDTMTP' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_CHCSubjectProfilePNDTMTP 
END
GO
CREATE PROCEDURE [dbo].[SPC_CHCSubjectProfilePNDTMTP] 
(
	@UserId INT
)
AS
