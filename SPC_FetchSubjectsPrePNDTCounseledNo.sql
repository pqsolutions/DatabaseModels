...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchSubjectsPrePNDTCounseledNo' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchSubjectsPrePNDTCounseledNo
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchSubjectsPrePNDTCounseledNo] 
(
	  @UserId INT
	 ,@DistrictId INT
	 ,@CHCId INT
...
BEGIN
	...
	AND PPC.[ANWSubjectId] NOT IN(SELECT ANWSubjectId FROM Tbl_PNDTestNew) AND PPC.[IsPNDTAgreeNo] = 1 AND PPC.[IsActive] = 1
