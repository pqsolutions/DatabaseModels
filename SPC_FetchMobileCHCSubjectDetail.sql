...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchMobileCHCSubjectDetail' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchMobileCHCSubjectDetail
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchMobileCHCSubjectDetail]
(
	@UserId INT
)AS
BEGIN
	...
	WHERE SP.[AssignANM_ID] = @UserId  AND SP.[UpdatedToANM] IS NULL
	AND SP.[RegisteredFrom] = (SELECT ID From Tbl_ConstantValues WHERE CommonName = 'CHC' AND comments = 'RegisterFrom')
	AND SP.[UniqueSubjectID] IN (SELECT UniqueSubjectID FROM Tbl_SubjectParentDetail)
END