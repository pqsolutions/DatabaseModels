...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchSubjectsForMolecularBloodTestComplete' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchSubjectsForMolecularBloodTestComplete
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchSubjectsForMolecularBloodTestComplete] 
(
	@MolecularLabId INT
)
AS
